...
from VisFly.utils.policies.sac_polices import SACPolicy, MultiInputPolicy
...
class SAC(sb_SAC):
    ...
    def __init__(self, comment="", save_path=None,scene_freq=None, *args, **kwargs):
        ...
        self.policy_save_path = f"{self.save_path}/SAC_{self.comment}" if comment is not None else f"{self.save_path}/SAC"
    ...
    def learn(
        ...
        try:
            with tqdm(total=total_timesteps, desc="SAC Training") as pbar:
                while self.num_timesteps < total_timesteps:
                    ...
                    pbar.update(max(0, self.num_timesteps - previous_timesteps))
        except KeyboardInterrupt:
            cache_path = self.policy_save_path + "_cache"
            self.save(cache_path)
            print(f"\n[SAC] KeyboardInterrupt detected, cache checkpoint saved.")
        finally:
