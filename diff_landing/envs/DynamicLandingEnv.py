...
class DynamicLandingEnv(DroneGymEnvsBase):
    def __init__(
            self,
        ...
        random_kwargs = {
            "state_generator":
                    ...
                    "class": "Uniform",
                    "kwargs": [
                        # {"position": {"mean": [2.0, 0.0, 2.0], "half": [1.5, 1.5, 1.0]}},
                        {"position": {"mean": [2.0, 0.0, 2.0], "half": [0.2, 0.2, 0.2]}},
                    ]
                    }
        }
        super().__init__(
        ...
        self.target_velocity = th.zeros((self.num_envs, 3), dtype=th.float32, device=self.device)
        self.centers = None
    ...
    def _sync_target_from_dynamic_platform(self) -> bool:
        """
        Sync per-scene landing target from dynamic platform position in SceneManager.
        Returns True if at least one scene target is updated from dynamic objects.
        """
        scene_manager = getattr(self.envs, "sceneManager", None)
        if scene_manager is None:
            return False

        dyn_positions = getattr(scene_manager, "dynamic_object_position", None)
        dyn_velocities = getattr(scene_manager, "dynamic_object_velocity", None)
        if not dyn_positions:
            return False

        updated = False
        for scene_id in range(self.num_scene):
            # SceneManager stores one entry per env (scene repeated num_agent_per_scene times).
            env_idx = scene_id * self.num_agent_per_scene
            if env_idx >= len(dyn_positions):
                continue

            obj_pos = dyn_positions[env_idx]
            if obj_pos is None:
                continue

            pos_tensor = th.as_tensor(obj_pos, device=self.device, dtype=th.float32)
            if pos_tensor.ndim == 2:
                # Use the first dynamic object as landing platform target.
                pos_tensor = pos_tensor[0]
            if pos_tensor.numel() < 3:
                continue

            pos_tensor = pos_tensor[:3].reshape(1, 3)
            pos_tensor[:, 2] += self.target_height_offset
            self.target[
                scene_id * self.num_agent_per_scene:(scene_id + 1) * self.num_agent_per_scene
            ] = pos_tensor.repeat(self.num_agent_per_scene, 1)

            if dyn_velocities and env_idx < len(dyn_velocities) and dyn_velocities[env_idx] is not None:
                vel_tensor = th.as_tensor(dyn_velocities[env_idx], device=self.device, dtype=th.float32)
                if vel_tensor.ndim == 2:
                    vel_tensor = vel_tensor[0]
                if vel_tensor.numel() >= 3:
                    vel_tensor = vel_tensor[:3].reshape(1, 3)
                    self.target_velocity[
                        scene_id * self.num_agent_per_scene:(scene_id + 1) * self.num_agent_per_scene
                    ] = vel_tensor.repeat(self.num_agent_per_scene, 1)
            else:
                self.target_velocity[
                    scene_id * self.num_agent_per_scene:(scene_id + 1) * self.num_agent_per_scene
                ] = 0.0
    ...
    def reset(self, indices: Optional[Union[List[int], th.Tensor]] = None, is_test=False) -> TensorDict:
        ...
        for i in range(self.envs.sceneManager.num_scene):
            ...
            self.target_velocity[i*self.num_agent_per_scene:(i+1)*self.num_agent_per_scene] = 0.0

        # Prefer dynamic platform position if object dynamics are enabled.
        self._sync_target_from_dynamic_platform()
    ...
    def get_observation(
        ...
        # Real-time target sync: keep landing target attached to moving platform.
        self._sync_target_from_dynamic_platform()
    ...
    def get_success(self) -> th.Tensor:
        ...
        rel_vel = self.velocity - self.target_velocity
        rel_xy_speed = rel_vel[:, :2].norm(dim=1)
        rel_z_speed = rel_vel[:, 2].abs()
    ...
    #     rel_vel = self.velocity - self.target_velocity
    #     d_xy = rel_pos[:, :2].norm(dim=1)
    #     d_z = rel_pos[:, 2].abs()
    #     rel_xy_speed = rel_vel[:, :2].norm(dim=1)
    #     rel_z_speed = rel_vel[:, 2]
    #     align_gate = th.exp(-4.0 * d_xy)
    #     near_gate = th.exp(-6.0 * d_xy - 10.0 * d_z)

    #     # 1. Put the drone over the moving platform and reduce height error.
    #     r_xy = -0.45 * d_xy
    #     r_height = -0.18 * d_z

    #     # 2. Match the moving platform's horizontal motion.
    #     r_rel_xy = -0.12 * rel_xy_speed

    #     # 3. Encourage a gentle descent relative to the platform once aligned.
    #     desired_rel_vz = -th.clamp(0.55 * d_z + 0.03, min=0.03, max=0.45)
    #     r_descent = align_gate * (1.0 / (1.0 + (rel_z_speed - desired_rel_vz).abs())) * 0.30
    #     r_rel_z = -0.06 * rel_z_speed.abs()

    #     # 4. Keep the attitude stable near touchdown.
    #     r_omega = (self.angular_velocity - 0).norm(dim=1) * -0.0015
    #     r_ori = (self.orientation - self.target_ori).norm(dim=1) * -0.004
    ...
    #     # 5. Vision and action smoothness.
    #     action_now = self._action.to(self.last_action.device)
    #     r_smooth = -0.0035 * action_now.norm(dim=1) - 0.0030 * (action_now - self.last_action).norm(dim=1)

    #     pixel_dist = (self.pixel_tensor[:,:2]).norm(dim=1)
    #     visible = self.pixel_tensor[:,2]
    #     r_visual = visible * (1.0 - pixel_dist) + (1.0 - visible) * -0.5
    #     r_visual = r_visual * 0.08

    #     # 6. Near-touchdown shaping: reward being centered, low and velocity-matched.
    #     touchdown_quality = 1.2 - 2.0 * d_xy - 2.0 * d_z - 0.6 * rel_xy_speed - 0.8 * rel_z_speed.abs()
    #     r_touchdown = near_gate * touchdown_quality.clamp(min=-1.0, max=1.5) * 0.8

    #     # 7. Terminal reward: explicit success bonus and collision/failure penalties.
    #     r_terminal = self.success.float() * 12.0 + self.failure.float() * -3.0 + self.is_collision.float() * -6.0

    #     diff_r = r_xy + r_height + r_rel_xy + r_descent + r_rel_z + r_omega + r_ori + r_yaw + r_smooth + r_visual + r_touchdown
    #     disc_r = r_terminal
    #     reward = diff_r + disc_r
    #     self.last_action = action_now.detach().clone()
    ...
    #         "r_z": r_height + r_descent + r_rel_z,
    #         "r_xy": r_xy,
    #         "r_visual": r_visual,
    #         "r_yaw": r_yaw,
    #         "r_rel_xy": r_rel_xy,
