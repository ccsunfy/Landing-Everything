...
class VisualLandingEnv(DroneGymEnvsBase):
    def __init__(
            ...
            semantic_normalize: bool = False,  # True: semantic /255，模拟真机侧归一化输入
            semantic_binary_mask: bool = True,  # True: 仅保留降落平台为 255，其余为 0（与真机二值 mask 对齐）
            semantic_platform_ids: Optional[Union[List[int], Tuple[int, ...]]] = None,
            # None 时默认 (255, 65, 67)：与当前 landing_* object_config 中平台 semantic_id 一致；若已全部改为 255 可传 [255]
    ):
        sensor_kwargs = [{
        ...
        random_kwargs = {
            "state_generator":
                    ...
                    "class": "Uniform",
                    "kwargs": [
                        {"position": {"mean": [1.3, 0.0, 2.0], "half": [0.5, 0.5, 0.5]}},
                    ]
                    }
                    # {
                    # "class": "Uniform",
        ...
        self.semantic_binary_mask = semantic_binary_mask
        self.semantic_platform_ids = tuple(
            semantic_platform_ids if semantic_platform_ids is not None else (255, 65, 67)
        )
    ...
    def _raw_semantic_to_binary_mask(self, semantic_raw: np.ndarray) -> np.ndarray:
        """真机对齐：降落平台像素为 255，背景为 0（float32）。"""
        # 避免 np.isin + int64 展平拷贝带来的额外峰值内存分配：
        # 直接用少量 id 的逐类比较，保持语义数组原 dtype。
        mask = np.zeros(semantic_raw.shape, dtype=bool)
        for pid in self.semantic_platform_ids:
            mask |= (semantic_raw == pid)
    ...
    def get_observation(
        ...
        semantic_raw = self.sensor_obs["semantic"]
        if self.semantic_binary_mask:
            semantic_obs = self._raw_semantic_to_binary_mask(semantic_raw)
        else:
            semantic_obs = semantic_raw.astype(np.float32)
        if self.semantic_normalize:
            semantic_obs = semantic_obs / 255.0
        if os.environ.get("DEBUG_SEMANTIC_STATS", "") and (self.total_timesteps % 100 == 0):
            sample = semantic_obs[0, 0]
            uniq = np.unique(sample)
            print(
                f"[semantic debug] dtype={semantic_obs.dtype}, shape={semantic_obs.shape}, "
                f"min={float(sample.min()):.1f}, max={float(sample.max()):.1f}, "
                f"unique_head={uniq[:10]}"
            )
    ...
    #     visible = self.pixel_tensor[:,2]
    #     r_visual = visible * (1.0 - pixel_dist) + (1.0 - visible) * -0.5
    #     r_visual = r_visual * 0.1    # - sensor_type: SEMANTIC从