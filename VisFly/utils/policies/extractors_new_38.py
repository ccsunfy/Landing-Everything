...
def _get_conv_output(net, shape):
    """在 CPU 上执行 probe，避免与 Habitat/OpenGL 的 CUDA 上下文冲突导致卡死"""
    net.eval()
    # 强制 CPU：避免与已有 GPU 渲染/OpenGL 冲突
    net_cpu = net.cpu()
    image = th.rand(1, *shape, device=th.device("cpu"))
    with th.no_grad():
        output = net_cpu(image)
    net.train()
    return output.numel()
...
def set_cnn_feature_extractor(cls, name, observation_space, net_arch, activation_fn):
    ...
    if backbone is not None:
        ...
        image_extractor = create_cnn(
            ...
            adaptive_avg_pool=net_arch.get("adaptive_avg_pool", True),
        )
    modules.append(image_extractor)
    cache_net = nn.Sequential(*modules)
    print(f"[set_cnn_feature_extractor] _get_conv_output for {name} shape={observation_space.shape}...", flush=True)
    conv_output = _get_conv_output(cache_net, observation_space.shape)
    print(f"[set_cnn_feature_extractor] _get_conv_output done, dim={conv_output}", flush=True)
    print(f"[set_cnn_feature_extractor] create_mlp input_dim={conv_output}...", flush=True)
    ...
    setattr(cls, name + "_extractor", net)
    cls._extract_names.append(name)
    print(f"[set_cnn_feature_extractor] done for {name}, output_dim={_output_dim}", flush=True)
    return _output_dim
...
class StateImageExtractor(BaseFeaturesExtractor):
    ...
    def __init__(self,
        ...
        assert ("state" in obs_keys) \
               and ("image" in obs_keys or "color" in obs_keys or "semantic" in obs_keys or "depth" in obs_keys)

        # 默认的特征维度为1
        self._features_dim = 1
        print(f"[StateImageExtractor] __init__ start id={id(self)}...", flush=True)
        super(StateImageExtractor, self).__init__(observation_space=observation_space,
                                                        features_dim=self.features_dim)

        print(f"[StateImageExtractor] state mlp...", flush=True)
        ...
        for key in observation_space.keys():
            if "image" in key or "color" in key or "semantic" in key or "depth" in key:
                print(f"[StateImageExtractor] cnn for {key}...", flush=True)
                _image_features_dims.append(set_cnn_feature_extractor(self, key, observation_space[key], net_arch.get(key, {}), activation_fn))
                self._image_extractor_names.append(key)
        self._features_dim = _state_features_dim + sum(_image_features_dims)
        print(f"[StateImageExtractor] _build done, features_dim={self._features_dim}", flush=True)
        if net_arch.get("recurrent", None) is not None:
            _hidden_features_dim = set_recurrent_feature_extractor(self, self._features_dim, net_arch.get("recurrent"))
            self._features_dim = _hidden_features_dim
            self._is_recurrent = True
    ...
    def forward(self, observations):
        ...
        for name in self._image_extractor_names:
            # image forward process (set_cnn_feature_extractor 注册为 name_extractor)
            image_obs = observations[name]
            # PPO path: semantic map is category-id image in [0, 255], normalize to [0, 1].
            if name == "semantic":
                image_obs = image_obs.to(th.float32) / 255.0
            x = getattr(self, name + "_extractor")(image_obs)
            features.append(x)
        # 合并特征
        if hasattr(self, "recurrent_extractor"):
