...
class SceneManager(ABC):
    ...
    def _object_step(self):
        for i in range(self.num_scene):
            ...
            if self.is_object_collidable:
                if hasattr(self.scenes[i], "update_dynamic_KDtree"):
    ...
    def _update_collision_infos(
        ...
        if velocities is None:
            velocities = [None for _ in range(len(indices))]
        ...
        for indice,vel in zip(indices, velocities):
            ...
            if self.col_refine_steps > 0 and vel is not None:
                col_records = []
    ...
    def load_scenes(self, indices=None):
        ...
        for scene_id in indices:
            ...
            if self.obj_settings:
                ...
                if hasattr(self.scenes[scene_id], "update_dynamic_KDtree"):
