...
class PPO(ori_PPO):
    ...
    def _setup_model(self) -> None:
        """Override to add debug prints for slow model creation."""
        # 原始实现（调用父类）
        # super()._setup_model()

        # 带调试打印的实现，用于定位卡住位置
        print("[PPO] _setup_model: lr_schedule + seed...", flush=True)
        self._setup_lr_schedule()
        self.set_random_seed(self.seed)
        from stable_baselines3.common.buffers import DictRolloutBuffer, RolloutBuffer
        if self.rollout_buffer_class is None:
            self.rollout_buffer_class = DictRolloutBuffer if isinstance(self.observation_space, spaces.Dict) else RolloutBuffer
        print("[PPO] _setup_model: 创建 rollout buffer (可能较慢)...", flush=True)
        ...
        print("[PPO] _setup_model: 创建 policy...", flush=True)
        self.policy = self.policy_class(
            self.observation_space, self.action_space, self.lr_schedule, use_sde=self.use_sde, **self.policy_kwargs
        )
        print("[PPO] _setup_model: policy.to(device)...", flush=True)
        self.policy = self.policy.to(self.device)
        # PPO 特有的 clip_range 等
        ...
        print("[PPO] _setup_model: 完成", flush=True)
    ...
    def save(
        ...
        index = 1
        save_path = f"{base_path}_{index}.zip"
