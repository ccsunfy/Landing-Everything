...
class DroneEnvsBase:
    ...
    # def _resolve_collision_penetration(self):
    #     if (not self.visual) or (self._collision_point is None) or (self._is_collision is None):
    #         return

    #     collided = th.where(self._is_collision)[0]
    #     if collided.numel() == 0:
    #         return

    #     pos = self.dynamics.position[collided]
    #     col_pt = self._collision_point[collided]
    #     delta = pos - col_pt
    #     dist = delta.norm(dim=1, keepdim=True)
    #     eps = 1e-6
    #     keepout = self.uav_radius + 1e-3

    #     normal = th.zeros_like(delta)
    #     valid = (dist.squeeze(1) > eps)
    #     if valid.any():
    #         normal[valid] = delta[valid] / dist[valid]

    #     vel = self.dynamics.velocity[collided]
    #     vel_norm = vel.norm(dim=1, keepdim=True)
    #     fallback = (~valid) & (vel_norm.squeeze(1) > eps)
    #     if fallback.any():
    #         normal[fallback] = -vel[fallback] / vel_norm[fallback]

    #     still_invalid = normal.norm(dim=1, keepdim=True).squeeze(1) <= eps
    #     if still_invalid.any():
    #         normal[still_invalid] = th.as_tensor([0.0, 0.0, 1.0], device=normal.device, dtype=normal.dtype)

    #     corrected_pos = col_pt + normal * keepout
    #     corrected_vel = vel.clone()
    #     inward_speed = (corrected_vel * normal).sum(dim=1, keepdim=True)
    #     corrected_vel = corrected_vel - th.minimum(inward_speed, th.zeros_like(inward_speed)) * normal

    #     with th.no_grad():
    #         self.dynamics._position[:, collided] = corrected_pos.T
    #         self.dynamics._velocity[:, collided] = corrected_vel.T
    ...
    def step(self, action):
        ...
        if self.visual and self._is_collision is not None and self._is_collision.any():
            # self._resolve_collision_penetration()
            self.sceneManager.set_pose(self.dynamics.position, self.dynamics._orientation.toTensor().T)
            self.update_collision()
